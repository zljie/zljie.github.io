...
import type { InteractionOption } from '../composables/useChat'
...
const { messages, inputValue, loading, messagesRef, sendMessage, scrollToBottom, resumeHitl } = useChat()
...
function handleHitlSelect(option: InteractionOption, params?: Record<string, any>) {
  if (option.action === 'confirm' || option.action === 'execute') {
    resumeHitl({ action: 'confirm', selectedValue: option.id })
  } else if (option.action === 'cancel') {
    resumeHitl({ action: 'cancel' })
  } else {
    // Other actions: use continueTask which creates a user bubble normally
    inputValue.value = `[action] ${option.id}`
    sendMessage()
  }
}

function handleHitlConfirm(id: string, _params?: Record<string, any>) {
  resumeHitl({ action: 'confirm', taskId: id })
}

function handleHitlCancel(id: string) {
  resumeHitl({ action: 'cancel', taskId: id })
}

function handleHitlRating(rating: number) {
  inputValue.value = `[rating] ${rating}`
  sendMessage()
}
