<template>
  <div class="md-bubble" :class="[`md-bubble--${role}`, { 'md-bubble--streaming': streaming }]">
    ...
    <HumanInTheLoopCard
      ...
      @select="(opt: InteractionOption, params?: Record<string, any>) => $emit('hitl-select', opt, params)"
      @confirm="(id: string, params?: Record<string, any>) => $emit('hitl-confirm', id, params)"
      @cancel="(id: string) => $emit('hitl-cancel', id)"
      @rating="(r: number) => $emit('hitl-rating', r)"
      @input="(v: string) => $emit('hitl-input', v)"
      @dismiss="$emit('hitl-dismiss')"
      @slot-fill="(id: string, values: Record<string, any>) => $emit('hitl-slot-fill', id, values)"
      @slot-cancel="(id: string) => $emit('hitl-slot-cancel', id)"
    />
