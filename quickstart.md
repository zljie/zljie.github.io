...
data: {"id": "TASK-001", "title": "补充订单信息", "message": "请补充以下信息以继续处理", "slots": [...], "continueAction": {...}, "cancelAction": {...}}
```

**格式二：紧凑格式（Tab 分隔）**
```
slot_fill_request	{"id": "TASK-001", "title": "补充订单信息", "message": "请补充以下信息以继续处理", "slots": [...], "continueAction": {...}, "cancelAction": {...}}
```
...
```json
{
  ...
  "slots": [
    {
      ...
      "validation": {
        "pattern": "^PO-\\d{4}-\\d+$",
        "patternMessage": "订单号格式应为 PO-年份-数字"
      }
    },
    {
      "id": "type",
  ...
  "riskLevel": "low",
  "alternatives": [
    { "label": "我想先查看所有未处理的需求", "value": "list_all" }
  ]
}
```
...
| type | 说明 | 支持的配置项 |
|------|------|-------------|
| `text` | 单行文本输入 | `placeholder`, `validation`（`pattern`, `patternMessage`, `minLength`, `maxLength`） |
| `textarea` | 多行文本输入 | `placeholder`, `maxLength` |
| `number` | 数字输入 | `placeholder`, `validation`（`min`, `max`） |
| `date` | 日期选择 | - |
...
function onSlotFill(id: string, values: Record<string, any>) {
  resumeHitl({
    action: 'submit_slots',
    taskId: id,
    filledSlots: values,  // 包含所有槽位字段，key 为 slot.id
  })
}
```
